...
class IntroToCharmingMcTesterCharm(ops.CharmBase):
    ...
    def __init__(self, *args):
        ...
        for event in self.on.events():
            self.framework.observe(event, self._holistic_handler)

    def _holistic_handler(self, _event: ops.EventBase):
        ...
        if ingress.relation:
            ...
            self.unit.status = ops.BlockedStatus("awaiting relation with demo-ingress-provider charm")
